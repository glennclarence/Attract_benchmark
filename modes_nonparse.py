...
def main():
    ...
    parser.add_argument("--na", "--rna", "--dna", dest="sugar",
                        help="Nucleic acid mode: use the sugar ring in the all-atom PDB file as center instead of the CA atom",
                        action="store_true")
    parser.add_argument("--scale", metavar="SCALE CONSTANT", help="Scale all spring force constants", type=float,
                        default=1.0)
    parser.add_argument("--aacontact",
                        help="Use the contacts from the all-atom PDB file to determine the spring constants (Setny et al.)",
                        action="store_true")
    ...
    head, tail = os.path.split(os.path.splitext(args.pdb)[0])
    name_output = tail + "_modes.dat"
    path_output = head

    modes(args.pdb, path_output, name_output, args.aapdb, args.nrmodes, args.sugar, args.scale, args.aacontact)

def modes( pdb, path_output, name_output, aapdb = None,nrmodes = 5, sugar= False, scale = 1.0, aacontact= False):
    outfile = path_output + name_output
    if sugar and not aapdb:
        raise ValueError("Nucleic acid mode requires an all-atom PDB reference provided with --aapdb")
    if aacontact and not aapdb:
        raise ValueError("--aacontact requires an all-atom PDB reference provided with --aapdb")

    coor, res, resn, atom = read_pdb(pdb)
    nr_res = len(set(res))

    if aapdb:
        coor_aa, res_aa, resn_aa, atom_aa = read_pdb(aapdb)
    ...
    for i in range(nrmodes):
        ...
        print >>file, " ", i + 1, float(s[n])
        for resindex in res:
            pos = 3 * (resindex - 1)
            #for n in range(3):
            print >>file, "{} {} {}".format( vec[pos ], vec[pos +1],vec[pos +2] )
            #print >>file

        print >>file
    file.close()

#modes( pdb="/home/glenn/Documents/Masterthesis/testfolder/benchmark_test/input/receptorr.pdb", path_output="/home/glenn/Documents/Masterthesis/testfolder/benchmark_test/input/" , name_output= "receptor_modasdes.dat", aapdb = None,nrmodes = 5, sugar= False, scale = 1.0, aacontact= False)