...
parser.add_argument("--batch", help="run reduce in batch mode. Input and output must be two lists of PDBs",
                    action="store_true")
...
def reduce(pdb, path_output, name_output, dna=False, rna=False, compat=False, chain=" ", startres=1, startatom=1,
        batch=False):
    ...
    if batch and (path_output is None and name_output is None):
    ...
    reducedat = os.environ['ATTRACTTOOLS'] + "/reduce.dat"
    ff = read_forcefield(reducedat)
    assert len(chain) == 1, chain

    if batch:
        infiles = read_filelist(pdb)
        for f in infiles:
        ...
        outp = open(outfile, "w")
        run(pdb,outp,ff,chain, startres, startatom, dna, rna,compat,mapping=True)
        print_res(outp,ff,chain, compat)
...
def print_res(outp,ff,chain,  compat, mapping=True ):
    ...
    for l in ff[resname]:
        ...
        if compat and resname in ("GLN", "GLU") and l[1] in ("CN1", "CO1"):
...
def run(pdb,outp,ff,chain, startres , startatom , dna , rna , compat,mapping=True):
    global res, resname, rescounter, atomcounter, rescoor
    res = None
    resname = None
    rescounter = startres - 1
    atomcounter = startatom - 1
    rescoor = {}
    ...
    for l in open(pdb):
        ...
        cres = l[21:26]
        if cres != res:
            print_res(outp,ff,chain,compat,mapping)
            res = cres
...
#reduce(pdb="/home/glenn/Documents/Masterthesis/testfolder/benchmark_test/input/receptor-aa.pdb", path_output="/home/glenn/Documents/Masterthesis/testfolder/benchmark_test/input/",
#       name_output = "receptor", dna=False, rna=False, compat=False, chain="A", startres=1, startatom=1,
#        batch=False)